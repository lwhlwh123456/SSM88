...
<%--<%
  ...
  if(o==null){
    out.print("<script>alert('你还没有登入，超时,可以滚啦');location.href='/page/login.jsp';</script>");
  }
%>--%>
...
function diao(obj,id) {
    var diao=$(obj).parent().parent().parent();

    $.post("/lll/delete",{"id":id},function (data) {
       if (data.result>0){
           diao.remove();
       }
    },"json");
...
<DIV class=search> 【欢迎：${usersname.name}】 <LABEL class="ui-green searchs"><a href="/lll/getHao" title="">发布房屋信息</a></LABEL><LABEL class="ui-green searchs"><a href="/lll/getHaoHouse" title="">查看房屋信息</a></LABEL>
...
<TABLE class=house-list>
  ...
  <c:forEach items="${pageInfo.list}" var="h">
    <TR>
      ...
      <TD class=house-type><LABEL class=ui-green><INPUT onclick="location.href='/lll/getUpdate?id=${h.id}'" value="修改" type=button name=search></LABEL></TD>
      <TD class=house-price><LABEL class=ui-green><INPUT onclick="diao(this,${h.id})" value="删    除" type=button name=search>
      </LABEL></TD>
    </TR>
  </c:forEach>
