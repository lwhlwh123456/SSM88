...
<HTML xmlns="http://www.w3.org/1999/xhtml"><HEAD><TITLE>青鸟租房 -发布房屋信息</TITLE>
    ...
    <script language="JavaScript">
        ...
        function showStreet(did){
            $.post("/ko/getMoeld",{"did":did},function(data){
                    ...
                    <TABLE class=field>
                        ...
                        <TR>
                            ...
                            <TD><SELECT class=text name=typeId>
                                <c:forEach items="${hao}" var="a">
                                    <OPTION <c:if test="${a.id==house.typeId}">selected="selected"</c:if> value=${a.id}>${a.name}</OPTION>
                                </c:forEach>
                            </SELECT>
                            </TD></TR>
                        <TR>
                        ...
                        <TR>
                            ...
                            <TD>区：<SELECT class=text id="district" name=districtId>
                                <c:forEach items="${hao1}" var="b">
                                    <OPTION  <c:if test="${house.distrctid==b.id}"> selected="selected"</c:if> value=${b.id}>${b.name}</OPTION>
                                </c:forEach>
                            </SELECT>
                                街：<SELECT class=text id="streetid" name=streetId>
                                    <OPTION selected value=1001>请选择</OPTION>
                                </SELECT> </TD></TR>
                        <TR>
                        ...
                        <TR>
                            ...
                            <TD><INPUT id=sss name="pfile" class=text type=file >
                                <img src="http://localhost:81/${house.path}" width="100" height="100"></TD></TR>
                        <TR>
                            <TD class=field>详细信息：</TD>
                            <TD><TEXTAREA name=description>${house.description}</TEXTAREA></TD></TR></TBODY></TABLE>
                    <DIV class=buttons><INPUT  value=修改发布 name="dd" type=submit>
